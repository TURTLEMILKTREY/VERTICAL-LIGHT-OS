...
async def demo_phase1_week1_2():
    ...
    print("🏗️  Initializing Hospital Data Intelligence System...")
    ...
    try:
        ...
        print(f"❌ Real-time data collection failed: {str(e)}")
        print()
    ...
    try:
        ...
        if 'financial' in metrics:
            ...
            print(f"      🛏️  Occupied Beds: {financial.get('occupied_beds', 0)}")
        
        if 'patient_flow' in metrics:
            flow = metrics['patient_flow']
            print(f"      🚪 Admissions: {flow.get('daily_admissions', 0)}")
            print(f"      🛏️  Occupancy Rate: {flow.get('bed_occupancy_rate', 0):.1%}")
            print(f"      ⏱️  Avg Length of Stay: {flow.get('average_length_of_stay', 0):.1f} days")
        ...
        if 'equipment_utilization' in metrics:
            ...
            print(f"      ⚙️  Equipment Downtime: {equipment.get('average_downtime_percentage', 0):.1f}%")
        ...
        print(f"❌ Data summary generation failed: {str(e)}")
        print()
    ...
    try:
        ...
        print(f"   ❌ HMS integration failed: {str(e)}")
    ...
    try:
        ...
        print(f"   ❌ Financial integration failed: {str(e)}")
    ...
    try:
        ...
        print(f"   ❌ Patient flow tracking failed: {str(e)}")
