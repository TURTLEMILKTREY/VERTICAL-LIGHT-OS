...
async def test_hospital_database_layer():
    ...
    try:
        ...
        if success:
            ...
            print("✗ Failed to save analysis record")
            return False
        ...
        if retrieved and retrieved.hospital_name == test_record.hospital_name:
            ...
            print("✗ Failed to retrieve analysis record")
            return False
        ...
        print(f"✗ HospitalDatabase layer test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
async def test_data_persistence_service():
    ...
    try:
        ...
        if result.success:
            ...
            print(f"✗ Service layer save failed: {result.error}")
            return False
        ...
        print(f"✗ DataPersistenceService test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
    ...
    try:
        ...
        if result.success:
            ...
            if saved_analysis:
                ...
                print("✗ Verification failed: Could not retrieve saved analysis")
                return False
        else:
...
async def main():
    ...
    if DATABASE_URL == 'postgresql://postgres:testpass@localhost:5432/hospital_test':
        print("⚠️  Using default database URL - make sure PostgreSQL is running")
    ...
    if not setup_ok:
        print("\n❌ Database setup failed. Please check connection and try again.")
    ...
    if overall_pass:
        ...
        print("\n❌ Integration tests failed - needs fixes before production")
