...
def start_collection(ctx, methods, priority_tiers, dry_run):
 ...
 if not dry_run:
 console.print("\n[bold yellow]WARNING: Starting data collection campaigns...")
...
def compare_hospital(ctx, hospital_id, metrics):
 ...
 status_icon = {
 'above_benchmark': 'COMPLETE',
 'near_benchmark': 'MEDIUM PRIORITY', 
 'below_benchmark': 'CRITICAL'
 }.get(data['status'], '⚪')
...
def status(ctx):
 ...
 # System health
 health_status = "COMPLETE Healthy" if status_data['database_status'] == 'healthy' else "CRITICAL Issues"
